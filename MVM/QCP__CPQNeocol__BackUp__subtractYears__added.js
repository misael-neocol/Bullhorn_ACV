...
fieldName === "NEO_Monthly_Net_Unit_Price_Primary__c") ||  fieldName === "Target_Monthly_Net_Unit_Price__c" && fieldName === SBQQ__UpgradedSubscription__c) {
return false;
}
...
function subtractFromYears() {
    ['NEO_Year_1_ACV__c', 'NEO_Year_2_ACV__c', 'NEO_Year_3_ACV__c', 'NEO_Year_4_ACV__c', 'NEO_Year_5_ACV__c'].forEach(yearField => {
...
if (quoteLine.record.Original_Subscription__c && quoteModel.record.Amendment_Quote_Type__c === "Replacement Quote") {
    subtractFromYears();
}
...
segmentPartial =
mrr * qlSegment.record.SBQQ__Quantity__c * (12 - offsetMonths - qlSegment.record.NEO_Subsequent_Segments_MRR_Additional__c) +
...
finalACV =
mrr * quantity * (12 - offsetMonths - qlSegment.record.NEO_Subsequent_Segments_MRR_Additional__c) +
(qlSegment.record.NEO_Previous_Segment_MRR__c * offsetMonths) +
(nextMRR * quantity * qlSegment.record.NEO_Subsequent_Segments_MRR_Additional__c) - 
...
if ((qlSegment.record.SBQQ__SegmentIndex__c > 1) && (qlSegment.record.Original_Subscription__c != null) && (qlSegment.record.SBQQ__UpgradedSubscription__c != null)) {
    //keeping this calc here for now, this variable isn't used anywhere though
    segmentPartial = previousMRR * previousQuantity * (12 - previousOffset) - mrr * quantity * offsetMonths;
    //calc for this year's ACV if it is an amended quote line:
    finalACV = acv + nextACV - previousAcv - qlSegment.record.NEO_Subscription_ACV__c;
    }
